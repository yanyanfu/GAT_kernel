...
array1d_t<int64_t> capsule_to_array1i(py::capsule& capsule) 
{
    //DLManagedTensor * dlMTensor = (DLManagedTensor *)PyCapsule_GetPointer(&input, "dltesnor");
    //The worst static_cast, this is becacuse of 'operator T*()' in pybind11 code
    //Pybind11 should have simply provided GetPointer() API.
    ...
    array1d_t<int64_t> array(data_ptr, shape0);
    return array;
}
...
PYBIND11_MODULE(kernel, m) {
    ...
    //     ;
        ...
        m.def("gspmm",
            [](py::capsule & s_row, py::capsule & s_col, py::capsule & s_value, py::capsule & d, py::capsule & output) {

                array1d_t <int64_t> s_row_array = capsule_to_array1i(s_row);
                array1d_t <int64_t> s_col_array = capsule_to_array1i(s_col);
                array1d_t <float> s_value_array = capsule_to_array1d(s_value);
                array2d_t <float> d_array = capsule_to_array2d(d);
                array2d_t <float> output_array = capsule_to_array2d(output);
                int64_t v_count = s_row_array.col_count-1;
                int64_t dim = d_array.col_count;

                return invoke_spmm(s_row_array.data_ptr, s_col_array.data_ptr, s_value_array.data_ptr, d_array.data_ptr, output_array.data_ptr, v_count, dim);
